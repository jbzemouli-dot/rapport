...
la résolution par moindres carrés pondérés et enfin l'exploitation de la
continuité temporelle par un filtre de Kalman étendu. Les modèles de correction
appliqués aux observations seront détaillés au chapitre~\ref{chaine_traitement_positionnement_gnss}.
...
elle est appelée \emph{pseudo-distance}.
...
Le modèle d'observation de phase s'écrit
...
Les mesures ne sont pas toutes de qualité identique. La phase est notamment
plus précise que le code, et les observations des satellites proches de
l'horizon sont souvent davantage perturbées. Ces différences sont décrites par
les statistiques des erreurs résiduelles, supposées centrées et regroupées
...
est souvent adoptée en première approche. L'écriture matricielle reste
toutefois générale et permet de représenter d'éventuelles corrélations par des
...
phase, le système linéarisé s'écrit explicitement
...
\begin{itemize}
    \item $\Delta\mathbf y$ le vecteur des écarts entre les observations et
    les valeurs calculées à l'état approché ;
    \item $\Delta\mathbf x$ le vecteur des corrections recherchées ;
    \item $\boldsymbol\varepsilon$ le vecteur des erreurs résiduelles ;
    \item $\mathbf H_0$ la matrice jacobienne du système complet.
\end{itemize}
