...
(TGD), diffusé dans le message de navigation. Il représente un biais de groupe propre au signal de code. Pour une
...
constellation : Galileo diffuse notamment des BGD. Le TGD ou le BGD doit donc
...
nouvelle constellation ajoute ainsi une inconnue d'ISB dans le vecteur d'état.
...
Le filtre de Kalman étendu est une autre méthode de résolution des équations.
Il exploite la continuité temporelle et fournit une estimation récursive : la solution
...
Le calcul récursif doit commencer avec une estimation initiale
...
hors diagonale de $\mathbf P_0$. C'est notamment le cas possible du biais
d'horloge de référence et d'un ISB construit par différence entre deux
horloges de constellation.

Une variance trop faible fige le paramètre autour d'une valeur initiale
potentiellement erronée ; une variance trop élevée ralentit sa convergence et
peut dégrader le conditionnement numérique. Les méthodes utilisées pour
...
phase dans le fichier RINEX. Le verrouillage désigne la capacité de la boucle
...
$\tau_j=t_j-t_k$. Un polynôme du second degré est ensuite ajusté par moindres
...
Lorsqu'un saut est détecté, l'époque courante marque le début d'un nouvel arc.
L'ambiguïté concernée est recalculée par
...
l'estimation ; une observation rejetée n'est utilisée ni par le WLS ni par
l'EKF.
...
c'est-à-dire déclarées saines, sont retenues. En l'absence d'éphéméride saine
pour l'époque considérée, toutes les observations du satellite sont écartées.
Avec des produits précis, le satellite doit disposer simultanément d'une
orbite SP3 et d'une horloge CLK valides sur l'intervalle traité.
...
ionosphère-libre exige les deux codes ; lorsque la phase est utilisée, les
...
Le contrôle des résidus permet de détecter des mesures aberrantes.
