...
C'est un estimateur adapté aux modèles dynamiques : la solution
obtenue à une époque devient l'information a priori de l'époque suivante. Le modèle d'évolution décrit la manière
dont l'état est susceptible de changer entre deux époques, tandis que le modèle
d'observation relie cet état aux mesures GNSS courantes. Cette méthode d'estimation suppose que le bruit des mesures est gaussien. Leurs incertitudes
respectives déterminent le poids accordé à la prédiction et aux observations.

L'indice $k$ désigne désormais l'époque. Une quantité prédite avant
l'utilisation des observations sera notée
$\widehat{\mathbf x}_k^-$. La quantité corrigée correspondante sera notée $\widehat{\mathbf x}_k$.

À chaque époque, le fonctionnement du filtre se décompose en quatre étapes :

\begin{enumerate}
    \item \textbf{Prédiction} : L'état et sa covariance sont propagés à partir
    de l'estimation corrigée à l'époque précédente,
    \item \textbf{Calcul de l'innovation} : Les observations reçues sont
    comparées aux mesures prévues par le modèle,
    \item \textbf{Calcul du gain de Kalman} : Le poids accordé à l'innovation
    est déterminé à partir des incertitudes de la prédiction et des mesures,
    \item \textbf{Correction} : L'état prédit et sa covariance sont mis à jour
    avant de servir de point de départ à l'époque suivante.
\end{enumerate}

\subsection{Initialisation du filtre}
\label{ann:sec:ekf_initialization}

Le calcul commence avec une estimation initiale
...
La matrice $\mathbf F_k$ traduit l'évolution du modèle attendue. Pour un
...
$\mathbf w_k$ représente les variations que ce modèle ne décrit pas.
...
Avant de traiter les nouvelles mesures, le filtre propage l'estimation et sa covariance obtenues
à l'époque précédente. L'ajout de $\mathbf Q_k$ augmente cette incertitude
