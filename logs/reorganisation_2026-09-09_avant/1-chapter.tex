...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Chapter 1 - Résolution
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\chapter{Implémentation}

\subsection{Théorie du positionnement GNSS}
...
$r$, le satellite $s$ et la fréquence $i$, elle peut être modélisée par
...
\begin{itemize}
    ...
    \item $\delta_{\mathrm{Shapiro},r}^s$ la correction relativiste de Shapiro ;
    \item $\varepsilon_{P,r,i}^s$ l'erreur résiduelle de la mesure.
\end{itemize}

Les différents termes de correction seront détaillés dans la
section~\ref{correction}. Après correction des termes pouvant être modélisés,
les principales inconnues sont la position du récepteur et son biais
d'horloge.

\subsection{Phase de la porteuse}

Le récepteur peut également mesurer la phase de la porteuse du signal reçu.
Cette observation est beaucoup plus précise que la pseudo-distance, car elle
repose sur le suivi de la phase d'une onde dont la longueur d'onde est de
l'ordre de quelques dizaines de centimètres pour les principales fréquences
GNSS.

La mesure ne fournit cependant que la partie fractionnaire de la phase lors de
l'acquisition initiale du signal. Le nombre entier de cycles déjà parcourus
entre le satellite et le récepteur est inconnu. Une ambiguïté de phase doit
donc être associée à chaque signal suivi de manière continue. Cette inconnue
rend l'exploitation des mesures de phase plus complexe, mais leur précision
permet d'atteindre des performances nettement supérieures à celles obtenues
avec les seules mesures de code.
...
Le filtre de Kalman étendu (ou \emph{Extended Kalman Filter}, EKF) est une autre méthode de résolution des équations établies en section~\ref{resolution}.
